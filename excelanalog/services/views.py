...
def svod(request):
    if request.method == 'POST':
        ...
        summary_df2 = pd.DataFrame(columns=['№ п/п', 'Код КП(промежуточный)', 'Исполнитель КП', 'номер чек листа',
...
'НУ-СВ-1-КП-001', 'НУ-СВ-1-КП-002', 'НУ-СВ-1-КП-003',]
        ...
        for file in files:
            ...
            df2 = df2.set_axis(['№ п/п', 'Код КП(промежуточный)', 'Исполнитель КП', 'номер чек листа',
                                            'Объект контроля (договор, акт, счет-фактура, КС-2 и др.)',
                                            'Дата документа', 'Номер документа', 'Количество документов/операций',
...
#    worksheet1.cell(row=9, column=11).value = checklist1.number_mistakes  =Sheet2!I24
    ...
    # Запись заголовков таблицы
    headers2 = ['№ п/п', 'Код КП(промежуточный)', 'Исполнитель КП', 'номер чек листа',
